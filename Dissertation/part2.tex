...
\end{equation}
где $N$ "--- количество нечетких правил, $A_{ki} \subseteq X_i, i=\overline{1,n}, B_k \subseteq Y$"--- нечеткие множества, которые характеризуются функциями принадлежности $\mu_{A_{ki}}(x_i)$ и $\mu_{B_k}(y)$ соответственно \cite{Zadeh1965FuzzySets}; $x_1, x_2,…,x_n$"--- входные переменные, причем
\[
...
\]
где $T_1$ - произвольная $t$-норма \cite{KlementMesiarPap2000TriangularNorms}, то правило \ref{eqn:fuz-problem-1} представляется в виде нечеткой импликации \cite{Shi2013, FernandezPeralta2025, Zhang2022, fern2021}
\begin{equation}
...
Несинглтонный фаззификатор \cite{Symmetry2021, Qian2023, Pekaslan2020} отображает измеренное $x_i=x'_i, i=\overline{1,n}$ в нечеткое число, для которого $\mu_{A'_i}(x'_i) = 1$ и $\mu_{A'_i}(x_i)$ уменьшается от единицы по мере удаления от  $x'_i$.
В соответствии с обобщенным нечетким правилом modus ponens \cite{Zadeh1973OutlineComplexSystems}, нечеткое множество $B'_k$ определяется композицией нечеткого множества $\mathbf{A'}$ и отношения $\mathbf{R_k}$, т.е.
\[
...
\subsection{Нечеткого значения истинности, когда функции принадлежности высказываний задаются гауссовыми функциями}
...
Используя правило истинностной модификации \cite{Dubois2012} можно выразить:
\[
...
\section{Выводы по главе \ref{ch:ch2}}
