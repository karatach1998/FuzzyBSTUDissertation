...
Нечеткая система на основе правил представляет собой базу правил вида \cite{RyzhovFuzzySets, rutkovskiy2010, NazarovKony2026}:
\begin{equation}
...
${}$\quad\enspace$A_{ki} \subseteq X_i, i=\overline{1,n}, B_k \subseteq Y$ --- нечеткие множества, которые характеризуются функциями принадлежности $\mu_{A_{ki}}(x_i)$ и $\mu_{B_k}(y)$ соответственно;\\
${}$\quad\enspace$x_1, x_2,…,x_n$"--- входные переменные, причем $[x_1, x_2, ..., x_n]^T = \mathbf{x} \in X_1 \times X_2 \times \dots \times X_n$.

Символами  $X_i, i=\overline{1,n}$ и $Y$ обозначаются соответственно пространства входных и выходной переменных. Если ввести обозначения $\mathbf{X}=X_1 \times X_2 \times \dots \times X_n$ и $\mathbf{A_k}=A_{k1}\times A_{k2} \times \dots \times A_{kn}$, причем функция принадлежности $\mu_{\mathbf{A_k}}$ \eqref{eqn:fuz-problem-5} выражается:
\begin{equation}
	\label{eqn:fuz-problem-5}
...
Правило $R_k$ для отношения \eqref{eqn:fuz-problem-2} можно формализовать как нечеткое отношение, определенное на множестве  $\mathbf{X}\times Y$, т.е. $R_k \subseteq \mathbf{X} \times Y$ - нечеткое множество с функцией принадлежности \eqref{eqn:fuz-problem-6}:
\begin{equation}
	\label{eqn:fuz-problem-6}
	\mu_{R_k}(\mathbf{x}, y) = \mu_{\mathbf{A_k} \to B_k} (\mathbf{x}, y).
\end{equation}

Модель логического типа определяет задание функции $\mu_{\mathbf{A_k} \to B_k} (\mathbf{x}, y)$ на основе известных функций принадлежности $\mu_{\mathbf{A_k}}(\mathbf{x})$ и $\mu_{B_k}(y)$ с помощью одной из функций нечеткой импликации \eqref{eqn:fuz-problem-7} \cite{RyzhovFuzzySets, rutkovskiy2010}:
\begin{equation}
	\label{eqn:fuz-problem-7}
...
$\mathbf{A'}=A'_1 \times A'_2 \times \dots \times A'_n \subseteq \mathbf{X}$ или $x_1\ \text{есть}\ A'_1\ \text{и}\ x_2\ \text{есть}\ A'_2\ \text{и} \dots \text{и}\ x_n\ \text{есть}\ A'_n$ с соответствующей функцией принадлежности $\mu_{\mathbf{A'}}(\mathbf{x})$, которая определяется формулой~\eqref{eqn:fuz-problem-3}:
\begin{equation}
	\label{eqn:fuz-problem-3}
	\mu_{\mathbf{A'}}(\mathbf{x}) = \underset{i=\overline{1,n}}{T_3} \mu_{A'_i}(x_i).
\end{equation}

Несинглтонный фаззификатор отображает измеренное $x_i=x'_i, i=\overline{1,n}$ в нечеткое число, для которого $\mu_{A'_i}(x'_i) = 1$ и $\mu_{A'_i}(x_i)$ уменьшается от единицы по мере удаления от  $x'_i$ \cite{Mendel2017, Pourabdollah2015}.
В соответствии с обобщенным нечетким правилом modus ponens, нечеткое множество $B'_k$ определяется композицией нечеткого множества $\mathbf{A'}$ и отношения $\mathbf{R_k}$, т.е. \eqref{eqn:fuz-problem-8} \cite{RyzhovFuzzySets, BorisovFuzzySets, rutkovskiy2010}:
\begin{equation}
	\label{eqn:fuz-problem-8}
...
Большинство имеющихся реализаций нечеткого вывода предоставляют возможность анализа лишь четких --- числовых данных \cite{Mendel2017, Pourabdollah2015}.

Нечеткая система представляется в виде композиции фаззификатора, базы правил, модуля вывода и дефаззификатора, как показано на рисунке \cref{fig:singleton-vs-nonsingleton}. Фаззификация --- отображение входных данных из исходного пространства в пространство нечетких множеств. Наиболее распространенным является метод синглтонной фаззификации. Основной причиной его широкого использования является существенное упрощение реализации систем нечеткого вывода. При использовании синглтонной фаззификации поданное на вход значение $x'$ интерпретируется как истинное значение измеренной величины, что эквивалентно использованию функции принадлежности входного нечеткого множества $\mu_{A'}(x) = \left[x = x'\right]$. При задании значений входных нечетких множеств нечеткой системы с использованием синглтонного типа фаззификации теряется информация о неопределенности измеренных значений $x'$ и они рассматриваются как достоверные \cite{RyzhovFuzzySets, Mendel2017, Pourabdollah2015}. Поэтому в работе используется \textit{несинглтонный способ фаззификации}.
...
Влияние перехода от синглтонной фаззификации к несинглтонной и величины окрестности погрешности на внутреннее поведение и итоговое качество нечеткой системы продемонстрировал Мендель в \cite{Mendel2021, Mendel2017}.

Разработке методов вывода для нечетких систем с лингвистическими входами и их параллельной реализации посвящены работы \cite{Sinuk2020, Karatach2021, Karatach2023}. В этих работах рассматриваются вопросы эффективной реализации нечетких систем с входами, заданными в виде лингвистических значений, что особенно актуально для задач обработки временных рядов с неопределенностью в исходных данных.

Для анализа влияния от использования того или иного способа фаззификации на корректность получаемого результата в статье \cite{Mendel2020} рассматривается карта разбиений первого и второго порядка на декартовом произведении базовых множеств входных лингвистических переменных, изображенная на рисунке \cref{fig:singleton-vs-nonsingleton-activations}. Мендель в своей книге проводил такое сравнение для систем типа Мамдани \cite{mamdani1975}. Поскольку в системах типа Мамдани в качестве функции импликации выступает $t$-норма некоторого конкретного вида \cite{Pourabdollah2015}, то разница от применения двух способов фаззификации проявляется в различных максимальных уровнях линии пересечения ф. п. входной посылки и антецедента правила. %В зависимости от минимум или произведение. 
...
\begin{figure}[ht]
	...
	{\small \noindent\hspace*{2.5em}\textls[300]{Примечание} --- Выходные нечеткие множества получены в результате применения импликации Лукасевича $I(a, b)=1-a+b$, поэтому для легкости восприятия антецеденты правил и входные нечеткие множества показаны в виде выражений $1-\mu_{A_k}(x)$ и $1-\mu_{A'}(x)$.}
	\end{minipage}
...
По мере удаления от входного значения $x$ возможность того, что оно будет правильным уменьшается. Таким образом, несинглтонные нечеткие множества могут эффективно улавливать входную неопределенность}}, не требуя изменений в других частях нечеткой системы, таких как антецеденты или консеквенты правил. Например, если несинглтонная входная ф. п. нечеткого множества, связанная с данным четким значением $x_0$, выражается гауссовой функцией \eqref{eqn:adaptive-fuz}:
\begin{equation}
	\label{eqn:adaptive-fuz}
	\mu(x) = \exp \left(x; x_0, b\right),
\end{equation}
где $b$ --- ширина или стандартное отклонение гауссовой ф. п., которая может быть задана для отражения уровня неопределенности. На практике, когда предполагается, что входные данные системы подвержены низким уровням неопределенности, ширина или значения $b$ соответствующих гауссовских ф. п. интуитивно мала, в то время как большие значения используются для моделирования более высоких уровней неопределенности \cite{Pourabdollah2017, Pekaslan2020}.

%Pekaslan2020
Специфика оценки шумовой неопределенности временной последовательности измерений возникает поскольку временной ряд отражает какую-то характеристику некоторого процесса, которому характерны различные виды переменчивости силы шумового воздействия. Примеры различных видов изменчивости уровня шума изображены на рисунке \cref{fig:noisy-ts-demo}. В \cite{Pekaslan2020} предложена обобщенная процедура \textit{адаптивной оценки неопределенности} для сопоставления значений исходной последовательности с несинглтонными ф. п. входных нечетких множеств. Данная процедура состоит из четырех шагов, записанных в блоках блок-схемы на рисунке \cref{fig:ns-procedure}:
...
\begin{figure}[tb!]
	...
	\end{minipage}
	\begin{minipage}{\linewidth}
	{\small \noindent\hspace*{2.5em}\textls[300]{Примечание} --- $SNR(dB) = 10 \log_{10} \frac{\sigma^2_s}{\sigma^2_n}$ --- отношение среднеквадратичного значения сигнала $\sigma^2_s$ к среднеквадратичному значению шума $\sigma^2_n$ (ОСШ, \textit{signal-to-noise ratio}) в децибелах. Большее значение $SNR(dB)$ соответствует меньшему уровню шума.}
	\end{minipage}
	
	\vspace{0.5em}
	\caption{Примеры временных рядов с различными типами шума: (а) стабильный шум, (б) смешанный стабильный шум, (в) переменный шум}
...
\begin{enumerate}
	\item \ul{Дефаззификация по методу среднего центра} вычисляется в центрах $\bar{y}_k$ правил, где функция принадлежности $k$-го консеквента $\mu_{B_k}(y_k)$ принимает максимальное значение \eqref{eqn:defuz-ca-1}:
	\begin{equation}
...
Сетевая структура нечеткой системы для данного способа дефаззификации изображена на рисунке \cref{fig:nfs-defuz-meom-with-defuz-demo}.
...
\begin{align}
	M[\flqq\text{истинно}\frqq]         &= \int_0^1 v/v, \label{eqn:ftv-cases-eq-1} \\
	M[\flqq\text{ложно}\frqq]         &= \int_0^1 1-v/v, \label{eqn:ftv-cases-eq-2} \\
	M[\flqq\text{слегка истинно}\frqq]   &= \int_0^1 \sqrt{v}/v, \label{eqn:ftv-cases-eq-3} \\
	M[\flqq\text{слегка ложно}\frqq]   &= \int_0^1 \sqrt{1-v}/v, \label{eqn:ftv-cases-eq-4} \\
	M[\flqq\text{очень истинно}\frqq]      &= \int_0^1 v^2/v, \label{eqn:ftv-cases-eq-5} \\
	M[\flqq\text{очень ложно}\frqq]    &= \int_0^1 \dfrac{(1-v)^2}{v}, \label{eqn:ftv-cases-eq-6} \\
	M[\flqq\text{абсолютно истинно}\frqq] & = \dfrac{1}{1} + \int_0^1 \dfrac{0}{v}, \label{eqn:ftv-cases-eq-7} \\
	M[\flqq\text{абсолютно ложно}\frqq] & = \dfrac{1}{0} + \int_0^1 \dfrac{0}{v}v, \label{eqn:ftv-cases-eq-8} \\
	M[\flqq\text{квазиистинно}\frqq]     &= \int_0^1 1/v, \label{eqn:ftv-cases-eq-9} \\
	M[\flqq\text{квазиложно}\frqq]     &= \int_0^1 0/v. \label{eqn:ftv-cases-eq-10}
\end{align}

В данной работе лингвистическая переменная <<истинность>> предлагается использовать для нечеткой оценки истинности одних нечетких высказываний относительно других. Значения этих высказываний формализуются нечеткими множествами $A$ и $A'$, определенными на базовом множестве $X$. Здесь высказывание соответствующее нечеткому множеству $A'$ рассматривается как достоверное, относительно которого оценивается истинность высказывания заданного нечетким множеством $A$.

\textbf{Определение.} Нечеткой истинностью множества $A$ относительно нечеткого множества $A'$ называется нечеткое множество $CP(A,A')$ \cite{Dubois2012} такое, что:
...
\begin{itemize}
	\item \textit{Аксиома истинности.} Нечеткое значение истинности ИСТИННО задается нечетким множеством \eqref{eqn:ftv-eq-case-true}:
	\begin{equation} 
		\label{eqn:ftv-eq-case-true}
	...
	На рисунке \cref{fig:ftv-gauss-true} представлены графики совпадающих функций принадлежности высказываний и построенной функции принадлежности нечеткого значения истинности.
	...
	\item \textit{Аксиома ложности.} Нечеткое значение истинности ЛОЖНО задается нечетким множеством \eqref{eqn:ftv-eq-case-false}:
	\begin{equation} 
		\label{eqn:ftv-eq-case-false}
		CP(A,A') = \left\{\langle\mu_{CP(A,A')}(v), v\rangle\right\} = \left\{(1-v)/v\right\} = \left\{v/(1-v)\right\}, v \in [0; 1],
	\end{equation}
	что выполняется тогда и только тогда, когда утверждаемое высказывание $A$ противоположно утверждаемому в $A'$, т. е. функции принадлежности высказываний $A'$ и $A$ удовлетворяют одному из условий \cref{eqn:ftv-eq-case-false-cond1, eqn:ftv-eq-case-false-cond2, eqn:ftv-eq-case-false-cond3}:
	\begin{equation}
	\label{eqn:ftv-eq-case-false-cond1}
	\mu_{A'}(x) = 1 - \mu_{A}(x)
	\end{equation}
	или
	\begin{equation}
	\label{eqn:ftv-eq-case-false-cond2}
	\mu_{A'}(x) = \left\{
	\begin{alignedat}{2}
	...
	или
	\begin{equation}
	\label{eqn:ftv-eq-case-false-cond3}
	\mu_{A'}(x) = \left\{
	\begin{alignedat}{2}
		0, &\quad x < x_{max} \\
	...
	На рисунке \cref{fig:ftv-gauss-false} представлены графики противоположных по значению функций принадлежности высказываний $A'$ и $A$ и построенной функции принадлежности нечеткого значения истинности.
	\item \textit{Аксиома абсолютной истинности.}  Значение истинности АБСОЛЮТНО ИСТИННО задается нечетким множеством \eqref{eqn:ftv-eq-case-absolute-true}:
	\begin{equation}
		\label{eqn:ftv-eq-case-absolute-true}
	...
	\item \textit{Аксиома абсолютной ложности.}  Значение истинности АБСОЛЮТНО ЛОЖНО задается нечетким множеством \eqref{eqn:ftv-eq-case-absolute-false}:
	\begin{equation}
		\label{eqn:ftv-eq-case-absolute-false}
	...
	На рисунке \ref{fig:ftv-gauss-absolute-false} представлены графики непересекающихся гауссовых функций принадлежности высказываний $A'$ и $A$ с удаленными центрами и построенная для этого случая функция принадлежности нечеткого значения истинности.
	...
	\item \textit{Аксиома квазиистинности.} Нечеткое значение истинности КВАЗИИСТИННО задается нечетким множеством \eqref{eqn:ftv-eq-case-quazi-true}:
	\begin{equation} 
		\label{eqn:ftv-eq-case-quazi-true}
	...
	На рисунке \cref{fig:ftv-gauss-quazi-true} представлены графики функций принадлежности нечеткого множества $A$, полностью содержащегося в нечетком множестве $A'$, и рассчитанной функции принадлежности нечеткого значения истинности.
	...
	\begin{figure}[ht]
		...
		\end{subfigure}
		\caption{Иллюстрация случая квазиистинного отношения высказываний}
		\label{fig:ftv-gauss-quazi-true}
	\end{figure}
	
	\item \textit{Аксиома квазиложности.} Нечеткое значение истинности КВАЗИЛОЖНО задается нечетким множеством \eqref{eqn:ftv-eq-case-quazi-false}:
	\begin{equation} 
		\label{eqn:ftv-eq-case-quazi-false}
	...
	Данную ситуацию нельзя выразить математически и изобразить, поскольку истинность функции принадлежности высказывания $A'$ не может быть оценена.
\end{itemize}

\subsection{Метод вывода на основе нечеткого значения истинности}

Используя правило истинностной модификации \cite{Dubois2012} можно выразить \eqref{eqn:ftv-compute-14}:
\begin{equation}
	\label{eqn:ftv-compute-14}
...
\begin{equation}
	\label{eqn:ftv-compute-15}
\mu_{A_k}(x) = v \textrm{ и } \mu_{A'}(x) = \tau_{A_k|A'}(v).
\end{equation}

Тогда для нечеткой системы с одним входом истинностное преобразование \eqref{eqn:ftv-compute-12} с учетом \eqref{eqn:ftv-compute-15} позволяет выполнить переход к новому виду выражения (\ref{eqn:fuz-problem-4}):
\begin{equation}
	\label{eqn:ftv-compute-5}
	\mu_{B'_k}(y|\mathbf{x'}) = \sup_{v \in [0,1]}\left\{\tau_{A_k|A'}(v) \overset{T_2}{\star} I(v, \mu_{B_k}(y))\right\}.
\end{equation}

Это соответствует новой структуре правил в базе правил, отличной от канонических структур Заде и Такаги-Сугено:
\begin{equation}
	\label{eqn:ftv-compute-13}
	\text{Если } \textit{нзи} \text{ есть } \text{ИСТИННО}, \text{ то }\ y\ \text{есть}\ B'_k.
\end{equation}

При переходе к нечеткому выводу по $n$ входам формула вычисления НЗИ для нечетких отношений посылки и факта имеет вид~\eqref{eqn:ftv-compute-16}:
\begin{equation}
	\label{eqn:ftv-compute-16}
...
\end{equation}
где $\mathrm{\tilde{T}}$ - расширенная по принципу обобщения $n$-местная $t$-норма \cite{kutsenko2015methods}, которая для \eqref{eqn:ftv-compute-7} определяется как
\begin{equation}
...
Рекурсивная схема вычисления свертки НЗИ по формуле (\ref{eqn:ftv-compute-8}) с учетом \eqref{eqn:ftv-compute-17} иллюстрируется выражением~\eqref{eqn:ftv-compute-10}:
\begin{gather}
...
Тогда для системы с $n$ входами выражение нечеткого вывода на основе НЗИ (\ref{eqn:ftv-compute-5}) примет вид:
\begin{equation}
...
В формуле (\ref{eqn:ftv-compute-11}) данный подход позволяет переместить процесс вывода в единое пространство НЗИ, где функции истинности, в отличие от различных пространств в подходе Заде, могут быть объединены в более эффективный вычислительный процесс.

Тогда, поскольку применение нового вида правила \eqref{eqn:ftv-compute-13} не зависит от количества входов в нечеткой системе, порядок функции временной сложности вычисления $B'_k$ на основе выражения (\ref{eqn:ftv-compute-11}) составляет $O\left(n|V|^2+|V|\cdot |Y|\right)$, где $V=[0;1]$. Сравнение схем нечетких выводов в соответствии с соотношениями (\ref{eqn:fuz-problem-4}) и (\ref{eqn:ftv-compute-11}) представлены на рисунке \cref{fig:ftv-schema-comparizon}.
...
Для использованной в работе дефаззификации по среднему максимуму вывод на основе нечеткого значения истинности выражается формулами \eqref{eqn:fuzzy-compute-tau} и \eqref{eqn:fuzzy-compute-output}:

\begin{equation}
	\label{eqn:fuzzy-compute-tau}
	\tau_{A_k|A'}(v) = \underset{i=\overline{1,n}}{\mathrm{\tilde{T}}} \left\{\mu_{CP(A_{ki}, A'_{t-p+i})}(v_i)\right\}, k=\overline{1,N},\\
\end{equation}
\begin{equation}
	\label{eqn:fuzzy-compute-output}
...
\begin{itemize}
	\item для \textit{S}-импликации~\eqref{fuzzy-cog-defuz-s-impl}
	\begin{equation}
		\label{fuzzy-cog-defuz-s-impl}
	...
	\end{equation}
	\item для \textit{R}-импликации~\eqref{fuzzy-cog-defuz-r-impl}
	\begin{equation}
		\label{fuzzy-cog-defuz-r-impl}
	...
	\end{equation}
	\item для \textit{Q}-импликации~\eqref{fuzzy-cog-defuz-q-impl}
	\begin{equation}
		\label{fuzzy-cog-defuz-q-impl}
		\hat{y}_{CoG} = \frac{
			% Numerator
			\begin{aligned}[t]
...
Методы вывода для систем типа Мамдани с несинглтонной фаззификацией были рассмотрены в работах \cite{Sinuk2022, Sinuk2023kii}. В этих работах анализируются особенности применения нечеткого вывода с учетом неопределенности входных данных для систем Мамдани, что является основой для разработанного в данной диссертационной работе метода прогнозирования временных рядов.

Вопросы классификации объектов на основе нечеткого вывода с конъюнктивным подходом при несинглтонной фаззификации рассматриваются в работе \cite{Karatach2022}, где предложен метод и параллельная реализация для решения задач классификации с учетом неопределенности входных данных.

В \cite{Sinuk2023} ослаблено ограничение на использование одной и той же $T$-нормы в формуле композиционного правила вывода в работах Менделя для случая вывода типа Мамдани, а также показано, что вывод по отдельному правилу в случае $T_2 = T_4 = T$ может быть записан через \textit{меру возможности} \eqref{eqn:fuzzy-output-mamdani-possibility} \cite{DuboisPrade1980FuzzySetsSystems}:
\begin{align}
	\label{eqn:fuzzy-output-mamdani-possibility}
...
Если при этом используется дефаззификация \textit{по центру сумм (CoS)} и $T$-норма Ларсена, то, как доказано в \cite{Sinuk2023}, результат дефаззификации зависит от ширины гауссовой или треугольной функции принадлежности консеквента, тогда как дефаззификация \textit{по среднему центру} учитывает только параметр центра. Например, при использовании в качестве выходной ф. п. гауссовой функции $\mu_{B_k}(y) = exp(-((y-\bar{y}_k)/\sigma_k)^2)$ формула дефаззификации имеет вид~\eqref{eqn:fuzzy-cos-defuz-larsen-impl}: 

\begin{equation}
	\label{eqn:fuzzy-cos-defuz-larsen-impl}
...
\unnumberedsection{Выводы по главе \ref{ch:ch2}}

\begin{enumerate}
	\item Приведенное описание нечеткого вывода логического типа для нечеткой системы на основе правил в обобщенном виде может быть использовано с несинглтонной фаззификацией. Нечеткий вывод при несинглтонной фаззификации позволяет учесть информацию о неопределенности входных данных, но имеет экспоненциальную вычислительную сложность.
